...
public class Point implements Comparable<Point> {
    ...
    public Comparator<Point> slopeOrder() {
        SlopeOrder slopeOrder = new SlopeOrder();
        return slopeOrder;
    }

    private class SlopeOrder implements Comparator<Point> {
        Point p = new Point(x, y);
        public int compare(Point q1, Point q2) { //'compare' returns -1. 0 or 1 depending on slopes.
            if (q1.slopeTo(p) < q2.slopeTo(p)) return -1;
            if(q1.slopeTo(p) > q2.slopeTo(p)) return 1;
            return 0;
        }
    }
